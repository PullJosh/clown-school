...
type Matrix = number[][];
type MatrixReducerState = Matrix;
type MatrixReducerAction =
  | {
      type: "setCell";
      x: number;
      y: number;
...
function matrixReducer(
  matrix: MatrixReducerState,
  action: MatrixReducerAction
): MatrixReducerState {
  switch (action.type) {
    case "setCell": {
        ...
        {hoveredPoint && (
          <div className="grid grid-cols-[auto,auto,auto,auto,auto] gap-y-[2px] text-center">
            {new Array(Math.max(matrixA[0].length, matrixB.length))
              .fill(null)
              .map((_, i) => [
                (matrixA[hoveredPoint.y]?.[i] ?? "?") as number | "?",
                (matrixB[i]?.[hoveredPoint.x] ?? "?") as number | "?",
              ])
              .map(([a, b], i) => (
                <Fragment key={i}>
                  <div
